...
{
   const char * sdot_kernel = "\n" \
         "__kernel void sdot(__global float* X, __global float* Y, __global float *res, int N, int incX, int incY) { \n" \
         ...
         "}";
   float res = 0;
   cblas_run_2vec_r(N, (void *)X, incX, (void *)Y, incY, (void *)&res, 0, 0, 0, NULL, sdot_kernel, "sdot");
   return res;
}   
