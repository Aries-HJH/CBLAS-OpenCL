...
{
   const char * cdotu_sub_kernel = "\n" \
         "__kernel void cdotu_sub(__global float* X, __global float* Y, __global float *dotu, int N, int incX, int incY) { \n" \
         ...
         "        shader_Y[id * 2] = X[gid * 2] * Y[gid * 2];\n" \
         "        shader_Y[id * 2 + 1] = X[gid * 2 + 1] * Y[gid * 2 + 1];\n" \
         ...
         "  } \n" \
         "}";
   cblas_run_2vec_r(N, (void *)X, incX, (void *)Y, incY, dotu, 0, 0, 1, NULL, cdotu_sub_kernel, "cdotu_sub");
}
